...
fig_1_dir <- file.path("fig/combined", paste0("fig_1.pdf"))
...
plot_response <- function(response_df, si_folder = NULL) {
  ...
  if (!is.null(si_folder) && 
      !si_folder %in% c("log_rate", "age_standardized")) {
    stop("si_folder must be either NULL, 'log_rate', or 'age_standardized'")
  }
...
############################################################################
plot1 <- plot_grid(
  plot_response(response_df = combined_response %>% filter(country == "US") %>% filter(model != "Pooled")),
...
fig_2_dir <- file.path("fig/combined", paste0("fig_2.pdf"))
...
ggsave(plot = log_rate_only_response, 
       filename = file.path("fig/combined", paste0("fig_2_log_rate.pdf")), 
       width = 12, 
       height = 12, 
       units = "in", 
       dpi = 300)
...
############################################################################
# SUPPLEMENTARY FIGIRE S2: AGE STANDARDIZED RATE (US and MEX)
############################################################################
...
ggsave(plot = age_std_rate_plot, 
       filename = file.path("fig/combined", paste0("fig_2_age_std_us_mex.pdf")),
       width = 12, 
       height = 12, 
       units = "in", 
       dpi = 300)
...
###################################################################################
# SUPPLEMENTARY FIGIRE S9: FIXED 1970 DECADE
###################################################################################
country_name = "US"
processed_country_dir <- paste("processed", country_name, sep = "/")
model_name <- "decade"
...
ggsave(
  ...
  filename = file.path("fig/combined", paste0("decade_fixed1970.pdf")), 
  width = 10, 
  height = 8, 
  units = "in", 
  dpi = 300

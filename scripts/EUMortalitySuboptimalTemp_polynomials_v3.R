...
# -----------------------------------
# 1. Prepare data: normalize names and making sure data is consistent
# -----------------------------------
names(mort)
mort <- mort %>% 
  mutate(across(starts_with("rate"), ~ .x/100000)) %>% # per person rate
  mutate(decade = floor(year/10)*10) %>% 
  mutate(decade = ifelse(decade == 2020, 2010, decade), # treat 2021 and 2022 and 2010s
         decade = as.factor(decade)) %>% 
  select(adm_id, year, week, starts_with("rate"),
         starts_with("n_deaths"), 
         starts_with("pop"), 
         starts_with("temp"), 
         starts_with("precip"), 
         decade, 
         mean_tm, # this is adm unit and year specific mean weekly temp
         temp_percentile, 
         mean_income, # this is adm unit and year specific mean inflated adjusted income
         income_percentile)

mort <- mort %>% 
  rename(
    pop_all = population_total, 
    pop_young = population_young, 
    pop_adult = population_adult, 
    pop_elderly = population_elderly, 
    rate_all = rate_total, 
    n_deaths_all = n_deaths_total)

length(unique(mort$adm_id)) 

# drop nas for rate_all columns
mort <- mort[!is.na(mort$rate_all), ]
length(unique(mort$adm_id))

# adjust weights for only europe as the data is unbalanced
# mort <- mort %>%
#   group_by(year, week) %>%
#   mutate(pop_all_weighted = pop_all / sum(pop_all, na.rm = T),
#          pop_young_weighted = pop_young / sum(pop_young, na.rm = T),
#          pop_adult_weighted = pop_adult / sum(pop_adult, na.rm = T),
#          pop_elderly_weighted = pop_elderly / sum(pop_elderly, na.rm = T)) %>%
#   ungroup()
...
pooled_model_formula <- as.formula("rate_all ~ 
                                   ...
                                   temp_d4_l0 + 
                                   precip_d1_l0 + 
                                   precip_d2_l0 |  adm_id^year + adm_id^week")
pooled_coefs <- feols(pooled_model_formula, data = mort, weights = NULL, only.coef = TRUE)
...
#                                    precip_d2_l0 + precip_d2_l.[1:30] |  adm_id^year + adm_id^week")
# 
...
# lag 1:30; zero is not included because we have no lag var in the main dataframe
lags <- 1:4
...
# EU data is weekly based so to include 30 days of lag effect, use only 3 weeks
pooled_model_formula_wlag_3week <- as.formula("rate_all ~ 
                                   temp_d1_l0 + temp_d1_t1 + temp_d1_t2 + temp_d1_t3 + temp_d1_t4 +
                                   temp_d2_l0 + temp_d2_t1 + temp_d2_t2 + temp_d2_t3 + temp_d2_t4 +
                                   temp_d3_l0 + temp_d3_t1 + temp_d3_t2 + temp_d3_t3 + temp_d3_t4 + 
                                   temp_d4_l0 + temp_d4_t1 + temp_d4_t2 + temp_d4_t3 + temp_d4_t4 +
                                   precip_d1_l0 + precip_d1_t1 + precip_d1_t2 + precip_d1_t3 + precip_d1_t4 + 
                                   precip_d2_l0 + precip_d2_t1 + precip_d2_t2 + precip_d2_t3 + precip_d2_t4 |  adm_id^year + adm_id^week")

pooled_coefs_wlag_3week <- feols(pooled_model_formula_wlag_3week, data = mort, weights = NULL, only.coef = TRUE)

# pooled_bstrap_wlag_3week <- run_bootstrap_feols(
#   sample_from = mort, 
#   unit_column = "adm_id", 
#   Nboot = Nboot, 
#   model_list = list(pooled_model_formula_wlag_3week), 
#   weights = NULL,
...
pooled_mmt_scaled_temp_wlag_3week <- find_mmt_and_scale_pred_temp(
  coefs = pooled_coefs_wlag_3week, 
  observed_temps = mort$temp_d1, 
  degree = 4, 
  temp_range = -50:50, 
  mmt_quantiles = c(0.01, 0.9), 
...
pooled_bstrap <- readRDS( paste(processed_country_dir, paste0(country_name, "_BootstrapResult_pooled_n100.rds"), sep = "/") )
...
# pooled w/ lag = 3w
pooled_bstrap_wlag_3week <- as.data.frame(t(pooled_coefs_wlag_3week))
# pooled_bstrap_wlag_3week <- readRDS( paste(processed_country_dir, paste0(country_name, "_BootstrapResult_pooled (lag)_n100.rds"), sep = "/") )
# pooled_bstrap_wlag_3week <- as.data.frame(pooled_bstrap_wlag_3week)

pooled_result_list_wlag_3week <- run_bootstrap_mortality_temp_analysis(
  boots_results = pooled_bstrap_wlag_3week,
  pred_temps_scaled = pooled_mmt_scaled_temp_wlag_3week$pred_temps_scaled,
  mmt <- pooled_mmt_scaled_temp_wlag_3week$mmt,
  pwd <- pooled_pwd,
  temp_range = -50:50,
  model_name = "Pooled (Lag)",
  model_level = "pooled",
...
pooled_response_df_wlag_3week <- pooled_result_list_wlag_3week$response_df
pooled_deaths_by_bins_wlag_3week <- pooled_result_list_wlag_3week$deaths_by_bins
pooled_deaths_by_side_iteration_wlag_3week <- pooled_result_list_wlag_3week$deaths_by_side_iteration
pooled_deaths_annual_by_side_wlag_3week <- pooled_result_list_wlag_3week$deaths_annual_by_side

# run Young model
young_model_formula <- as.formula("rate_young ~ 
                                  temp_d1_l0 + temp_d1_t1 + temp_d1_t2 + temp_d1_t3 +
                                  ...
                                  precip_d2_l0 + precip_d2_t1 + precip_d2_t2 + precip_d2_t3 |  adm_id^year + adm_id^week")
...
# young_bstrap <- as.data.frame(t(young_coefs))
young_bstrap <- readRDS( paste(processed_country_dir, paste0(country_name, "_BootstrapResult_young_n100.rds"), sep = "/") )
...
adult_bstrap <- readRDS( paste(processed_country_dir, paste0(country_name, "_BootstrapResult_adult_n100.rds"), sep = "/") )
...
elderly_bstrap <- readRDS( paste(processed_country_dir, paste0(country_name, "_BootstrapResult_elderly_n100.rds"), sep = "/") )
...
# run Temp Continuous Interaction model
temp_model_formula <- as.formula("rate_all ~ 
                                  temp_d1_l0 + temp_d1_t1 + temp_d1_t2 + temp_d1_t3 +
                                  temp_d2_l0 + temp_d2_t1 + temp_d2_t2 + temp_d2_t3 + 
                                  temp_d3_l0 + temp_d3_t1 + temp_d3_t2 + temp_d3_t3 + 
                                  temp_d4_l0 + temp_d4_t1 + temp_d4_t2 + temp_d4_t3 + 
                                ...
                                ( temp_d1_l0 + temp_d1_t1 + temp_d1_t2 + temp_d1_t3 +
                                  ...
                                  temp_d4_l0 + temp_d4_t1 + temp_d4_t2 + temp_d4_t3 + 
                                  precip_d1_l0 + precip_d1_t1 + precip_d1_t2 + precip_d1_t3 +  
                                  precip_d2_l0 + precip_d2_t1 + precip_d2_t2 + precip_d2_t3 ):mean_tm | adm_id^year + adm_id^week")
temp_coefs <- feols(temp_model_formula, data = mort, weights = NULL, only.coef = TRUE)
...
temp_bstrap <- readRDS( paste(processed_country_dir, paste0(country_name, "_BootstrapResult_temp_n100.rds"), sep = "/") )
...
income_model_formula <- as.formula("rate_all ~ 
                                  temp_d1_l0 + temp_d1_t1 + temp_d1_t2 + temp_d1_t3 +
                                  temp_d2_l0 + temp_d2_t1 + temp_d2_t2 + temp_d2_t3 + 
                                  temp_d3_l0 + temp_d3_t1 + temp_d3_t2 + temp_d3_t3 + 
                                  temp_d4_l0 + temp_d4_t1 + temp_d4_t2 + temp_d4_t3 + 
                                  ...
                                  temp_d4_l0 + temp_d4_t1 + temp_d4_t2 + temp_d4_t3 + 
                                  precip_d1_l0 + precip_d1_t1 + precip_d1_t2 + precip_d1_t3 +  
                                  precip_d2_l0 + precip_d2_t1 + precip_d2_t2 + precip_d2_t3):mean_income | adm_id^year + adm_id^week")
...
income_bstrap <- readRDS( paste(processed_country_dir, paste0(country_name, "_BootstrapResult_income_n100.rds"), sep = "/") )
...
str(mort)
names(mort)
decade_model_formula <- as.formula("rate_all ~ 
                                   i(decade, temp_d1_l0) + i(decade, temp_d1_t1) + i(decade, temp_d1_t2) + i(decade, temp_d1_t3) + 
                                   i(decade, temp_d2_l0) + i(decade, temp_d2_t1) + i(decade, temp_d2_t2) + i(decade, temp_d2_t3) + 
                                   i(decade, temp_d3_l0) + i(decade, temp_d3_t1) + i(decade, temp_d3_t2) + i(decade, temp_d3_t3) + 
                                   i(decade, temp_d4_l0) + i(decade, temp_d4_t1) + i(decade, temp_d4_t2) + i(decade, temp_d4_t3) + 
                                   i(decade, precip_d1_l0) + i(decade, precip_d1_t1) + i(decade, precip_d1_t2) + i(decade, precip_d1_t3) + 
                                   i(decade, precip_d2_l0) + i(decade, precip_d2_t1) + i(decade, precip_d2_t2) + i(decade, precip_d2_t3) | adm_id^week + adm_id^year")
...
decade_bstrap <- readRDS( paste(processed_country_dir, paste0(country_name, "_BootstrapResult_decade_n100.rds"), sep = "/") )
...
# run Decade model on full/panel subset (data available for each year) data
mort[, n_years := uniqueN(year[!is.na(rate_all)]), by = adm_id]
fdecade_mort <- mort[n_years == 23]

fdecade_model_formula <- as.formula("rate_all ~
                                   i(decade, temp_d1_l0) + i(decade, temp_d1_t1) + i(decade, temp_d1_t2) + i(decade, temp_d1_t3) + 
                                   i(decade, temp_d2_l0) + i(decade, temp_d2_t1) + i(decade, temp_d2_t2) + i(decade, temp_d2_t3) + 
                                   i(decade, temp_d3_l0) + i(decade, temp_d3_t1) + i(decade, temp_d3_t2) + i(decade, temp_d3_t3) + 
                                   i(decade, temp_d4_l0) + i(decade, temp_d4_t1) + i(decade, temp_d4_t2) + i(decade, temp_d4_t3) + 
...
fdecade_coefs <- feols(fdecade_model_formula, data = fdecade_mort, weights = NULL, only.coef = T)
# fdecade_bstrap <- run_bootstrap_feols(
#   sample_from = fdecade_mort, 
#   unit_column = "adm_id", 
#   Nboot = Nboot, 
#   model_list = list(fdecade_model_formula), 
#   weights = NULL,
#   model_name = "full_decade",
#   country_name = country_name,
#   processed_country_dir = processed_country_dir,
#   override = T
# )
fdecade_bstrap <- as.data.frame(t(fdecade_coefs))
fdecade_results_list <- run_parallel_mortality_analysis(
  mort = fdecade_mort, 
  pwd = decade_pwd, 
  coefs = fdecade_coefs, 
  boots_results = fdecade_bstrap, 
  temp_range = -50:50, 
  model_name = "decade", 
  country_name = country_name, 
  coef_pattern = "temp",
...
fdecade_response_df <- fdecade_results_list$response_df
fdecade_deaths_by_bins <- fdecade_results_list$deaths_by_bins
fdecade_deaths_by_side_iteration <- fdecade_results_list$deaths_by_side_iteration
fdecade_deaths_annual_by_side <- fdecade_results_list$deaths_annual_by_side
...
pooled_response_df_wlag_3week <- read_parquet(file.path(processed_country_dir, paste0(country_name, "_MortalityTempAnalysis_pooled (lag)_response_by_bins.pq")))
...
pooled_deaths_annual_by_side_wlag_3week <- read_parquet(file.path(processed_country_dir, paste0(country_name, "_MortalityTempAnalysis_pooled (lag)_deaths_by_side_year.pq")))
...
plot_response <- function(response_df, min_bins, max_bins, max_pop) {
  ...
  # response_df <- pooled_response_df
  # min_bins = 0
  # max_bins = 40
  # 
...
age_response_plot[[1]] +
  facet_wrap(~model_level, scale = 'free_y')

# pooled_plot | age_response_plot
# sum(pooled_response_df$pop_deg_days)
# sum(young_response_df$pop_deg_days)
...
decade_response_plot <- rbind(decade_response_df, fdecade_response_df %>% mutate(model_name = "full decade")) %>% 
  mutate(model_level = case_when(
  ...
  mutate(model_level = case_when(
    model_level == "decade 2000s" ~ "2000s",
    model_level == "decade 2010s" ~ "2010s",
    model_level == "full decade 2000s" ~ "Full 2000s",
    model_level == "full decade 2010s" ~ "Full 2010s"
  )) %>% 
  plot_response(temp_min, temp_max, max_pop)

# Prepare decade annual deaths plot with recoded model levels
decade_deaths_result <- rbind(decade_deaths_annual_by_side, fdecade_deaths_annual_by_side %>% mutate(model_name = "full decade")) %>% 
  mutate(model_level = case_when(
    model_level == 2000 ~ "2000s",
    model_level == 2010 ~ "2010s"
  )) %>% 
...
final_plot <- plot_grid(responses_row, deaths_row, ncol = 1, align = "vh")
final_plot
plot_out_dir <- file.path(fig_country_dir, paste0(country_name, "_MortalityTempAnalysis_combined_plot.png"))
ggsave(plot = final_plot, filename = plot_out_dir, width = 20, height = 8, units = "in", dpi = 300)
